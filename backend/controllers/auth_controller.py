...
class AuthController:
    ...
    def google_login(self) -> Tuple[Dict[str, Any], int]: 
        ...
        data = request.json 
        # Buscar tanto 'credential' como 'token' para compatibilidad
        id_token = data.get('credential') or data.get('token')
        ...
        try:
            ...
            if not JWT_AVAILABLE:
                logger.error("PyJWT no está disponible para decodificar el token.")
                return jsonify({"message": "Funcionalidad de Google Login no disponible."}), 503
